...
async function getAllActivity() {
  let projectAllID = getProjectID();
    try{
      ...
      data.data.datas.sort((a, b) => new Date(b.acted_on) - new Date(a.acted_on));
  
      // Group activities by date
      const activitiesByDate = data.data.datas.reduce((acc, acti) => {
        const date = new Date(acti.acted_on).toLocaleDateString("en-US", {
          year: 'numeric',
          month: 'long',
          day: 'numeric'
        });
      ...
      for ([date, activities] of Object.entries(activitiesByDate)) {
        let displayDate;
        ...
        showAllactivity.innerHTML += `
          <div class="date-header mb-3 ms-4">
            <strong class="text-primary">${displayDate}</strong>
          </div>`;
  
        activities.forEach(acti => {
          const formattedDate = new Date(acti.acted_on).toLocaleString("en-US", {
            month: "short",
            day: "2-digit",
            year: "numeric",
            hour: "2-digit",
          ...
          showAllactivity.innerHTML += `
            <div class="activity-card px-3 py-2">
              <div class="activity-item ps-0 m-0">
                <div class="activity-hover">
                  <i class='bx bx-user fs-4 pe-2'></i>
                  <strong>${acti.actor.user.first_name} ${acti.actor.user.last_name}</strong> <span class="text-primary fw-bold">${acti.actor.user.role.name}</span> ${acti.activity}
                  <strong>${acti.project.name}</strong> ${acti.title}
                  <span class="text-muted">${formattedDate}</span>
                </div>
                <div class="btn-hover-delete">
                  <button class="delete-btn m-0 p-0" data-id="${acti.id}" onclick="deleteActivity(this)">
...
async function deleteActivity(button) {
    ...
    try {
        ...
        if (result.isConfirmed) {
          const response = await fetch(`${baseApiUrl}/api/projects/activity/${idActivity}`, {
            method: "DELETE",
          });
          ...
          await Swal.fire({
            title: "Deleted!",
            text: "Delete activity sucessfully",
            icon: "success",
          });
        }
      } catch (error) {
