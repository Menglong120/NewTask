const fetchProjectProgress = async function() {
    try {
        const response = await fetch(`${baseApiUrl}/api/analyst/dashboard/allprogress`, {
            method: 'GET',
            headers: {
...
const renderProjects = function(projects) {
    ...
    projects.forEach(pr => {
        const progressInt = parseInt(pr.progress);
        let progressColor = pr.progress < 30 ? 'bg-danger' : pr.progress < 70 ? 'bg-warning' : 'bg-success';

        let projectWrapper = document.createElement('div');
        projectWrapper.classList.add('mb-3');

        let projectInfo = document.createElement('div');
        projectInfo.classList.add('d-flex', 'justify-content-between', 'mb-1');
        projectInfo.innerHTML = `<span>${pr.name}</span><span>${progressInt}%</span>`;

        let progressContainer = document.createElement('div');
        progressContainer.classList.add('progress');

        let progressBar = document.createElement('div');
        progressBar.classList.add('progress-bar', progressColor);
        progressBar.setAttribute('role', 'progressbar');
        progressBar.setAttribute('aria-valuenow', pr.progress);
        ...
        projectWrapper.appendChild(progressContainer);
        showPro.appendChild(projectWrapper);

        setTimeout(() => progressBar.style.width = `${progressInt}%`, 100);
    });
};

const loadProjectProgress = async function() {
